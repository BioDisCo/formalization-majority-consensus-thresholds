...
/-- Paper `thm:nsd-intra` Part 1, generalized to arbitrary β,δ ≥ 0.
    Under the hypotheses of `thm_nsd_intra_general`, consensus is reached a.s.
    and Pr[MCE] = (a/(a+b)) · Pr[consensus reached]. Since consensus is certain,
    this is equivalent to Pr[majority consensus | consensus] = a/(a+b). -/
theorem thm_nsd_intra_part1_general
    (params : LVParams)
    (hAlpha : 0 < params.alpha0)
    (hNeutral : params.alpha0 = params.alpha1)
    (hEq0 : params.gamma0 = 2 * params.alpha0)
  ...
  have hAlphaSum : 0 < params.alpha0 + params.alpha1 := by
    linarith [hNeutral.symm ▸ hAlpha]
  have hcons := nsd_consensus_almost_sure_general params
    hGamma0 hGamma1 hAlphaSum a b hposA hposB hAlpha hNeutral hEq0 hEq1
  constructor
  · rw [hcons]; exact one_pos
  · rw [hcons, mul_one]
    exact thm_nsd_intra_general params hAlpha hNeutral hEq0 hEq1 a b hposA hposB hba

/-- Exact corrected paper theorem: under `γᵢ = 2αᵢ` and positive neutral
interspecific competition, consensus is almost sure and the initial species-0
fraction is its winning probability. -/
theorem thm_nsd_intra_full_general
    (params : LVParams)
    (hAlpha : 0 < params.alpha0)
    (hNeutral : params.alpha0 = params.alpha1)
    (hEq0 : params.gamma0 = 2 * params.alpha0)
  ...
  have hAlphaSum : 0 < params.alpha0 + params.alpha1 := by
    linarith [hNeutral.symm ▸ hAlpha]
  exact
    ⟨nsd_consensus_almost_sure_general params hGamma0 hGamma1 hAlphaSum
        a b hposA hposB hAlpha hNeutral hEq0 hEq1,
      thm_nsd_intra_general params hAlpha hNeutral hEq0 hEq1
        a b hposA hposB hba⟩
